...
@interface SMFormFieldFactoryTests : SenTestCase
...
- (void)testFactoryShouldFailIfFieldTypeNotSet
{
    ...
    STAssertNil(field, @"field must be nil");
}
...
- (void)testFactoryCreateTextFields
{
    ...
    STAssertTrue([field isKindOfClass:[SMFormField class]], @"form field should be a form field");
    STAssertTrue([field isKindOfClass:[SMFormTextField class]], @"form field should be a text field");
    STAssertTrue([[field name] isEqualToString:@"username"], @"field name must set");
}
